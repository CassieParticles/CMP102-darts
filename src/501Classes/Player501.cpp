...
Player501::Player501(std::string name, float bullseyeChance, float normalHitChance, int startingScore, Dartboard501* dartBoard)
...
int Player501::hit(Target target)	//Handles the player hitting a specific target, deals with score going below 50
{
	...
	if (currentScore - target.sector == 0) //Must have hit a bullseye
	{ 
		bullseyeCount++;
		return currentScore -= target.sector; //Subtracts the target from the score, (will always be 0), then returns that 0
	}
	if (currentScore - target.sector < dartBoard->getBullseyeValue()) //Non-bullseye takes it below 50, invalid and disregarded
	{
		//std::cout << "Cannot go below "<< dartBoard->getBullseyeValue()<<", must end on a bullseye\n";
		return currentScore;
	}	
	return currentScore -= target.sector;
}
